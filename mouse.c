...
#include "interrupt.h"
#include "printk.h"
struct keyboard_inputbuffer* p_mouse=0;
...
void mouse_handler(unsigned long nr,unsigned long parameter,struct pt_regs* regs){
	unsigned char x;
	x=io_in8(0x60);
	color_printk(WHITE,BLACK,"(M:%#02x)",x);
	if(p_mouse->p_head==p_mouse->buf+KB_BUF_SIZE)	p_mouse->p_head=p_mouse->buf;
	*p_mouse->p_head=x;
	p_mouse->count++;
	p_mouse->p_head++;
}
struct hw_int_controller mouse_int_controller={
...
unsigned char get_mousecode(){
	unsigned char ret=0;
	while(!p_mouse->count)	nop();
	if(p_mouse->p_tail==p_mouse->buf+KB_BUF_SIZE)	p_mouse->p_tail=p_mouse->buf;
	ret=*p_mouse->p_tail;
	p_mouse->count--;
	p_mouse->p_tail++;
	return ret;
